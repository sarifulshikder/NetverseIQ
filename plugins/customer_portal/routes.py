...
def get_router(injected_models: Dict[str, Any]):
    ...
    async def portal_login(phone: str):
        # Mock OTP login
    ...
    async def portal_dashboard(customer_id: int, db: AsyncSession = Depends(_get_db)):
    ...
    async def portal_invoices(customer_id: int, db: AsyncSession = Depends(_get_db)):
    ...
    async def portal_tickets(customer_id: int, db: AsyncSession = Depends(_get_db)):
        return [
