...
def get_router(injected_models: Dict[str, Any]):
    ...
    async def get_all_settings(db: AsyncSession = Depends(_get_db)):
        result = await db.execute(select(Setting))
        return result.scalars().all()

    @router.get("/{group}", summary="Get settings by group")
    async def get_settings_by_group(group: str, db: AsyncSession = Depends(_get_db)):
        result = await db.execute(select(Setting).where(Setting.group == group))
        return result.scalars().all()

    @router.put("/{key}", summary="Update setting by key")
    async def update_setting(key: str, body: Dict[str, Any], db: AsyncSession = Depends(_get_db)):
    ...
    async def get_company_profile(db: AsyncSession = Depends(_get_db)):
    ...
    async def update_company_profile(body: Dict[str, Any], db: AsyncSession = Depends(_get_db)):
        result = await db.execute(select(CompanyProfile).limit(1))
        profile = result.scalar_one_or_none()
        if not profile:
    ...
    async def list_sms_gateways(db: AsyncSession = Depends(_get_db)):
        result = await db.execute(select(SMSGateway))
        return result.scalars().all()

    @router.post("/sms/test", summary="Test SMS gateway")
    async def test_sms(body: Dict[str, Any]):
